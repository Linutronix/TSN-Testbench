...
static void LogViaMQTTAddTrafficClass(struct mosquitto *mosq, const char *MQTTBaseTopicName, struct LogStatistics *stat)
{
    ...
    size_t statMessageLength = sizeof(statMessage) - 1;
    p = statMessage;

    uint64_t timeNs = stat->TimeStamp;
...
{
    printf("MQTT Log Info onConnect: %s\n", mosquitto_connack_string(reason_code));
    if (reason_code != 0)
    {
...
static void *LogViaMQTTThreadRoutine(void *data)
{
    ...
    int connectStatus = mosquitto_connect(mqttContext->mosq, appConfig.LogViaMQTTBrokerIP,
                                          appConfig.LogViaMQTTBrokerPort, appConfig.LogViaMQTTKeepAliveSecs);
    ...
    while (!mqttContext->Stop)
    {
        ...
        int nofReadElements = logDataLen / sizeof(struct LogStatistics);
        struct LogStatistics *currStats = (struct LogStatistics *)stats;
