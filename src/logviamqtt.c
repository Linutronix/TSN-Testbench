...
void LogViaMQTTStats(enum StatFrameType frameType, struct Statistics *stats)
{
	...
	RingBufferAdd(LogViaMQTTGlobalLogRingBuffer, (const unsigned char *)&internal,
		      sizeof(struct LogStatistics));
}

static void LogViaMQTTAddTrafficClass(struct mosquitto *mosq, const char *MQTTBaseTopicName,
				      struct LogStatistics *stat)
{
	...
	written = snprintf(p, statMessageLength,
			   ...
			   "stats", StatFrameTypeToString(stat->FrameType), stat->FramesSent,
			   stat->FramesReceived, stat->RoundTripMin, stat->RoundTripMax,
			   stat->RoundTripAvg, stat->OutOfOrderErrors, stat->FrameIdErrors,
			   stat->PayloadErrors, stat->RoundTripOutliers);
	...
	resultPub = mosquitto_publish(mosq, NULL, "testbench", strlen(statMessage), statMessage, 2,
				      false);
...
static void *LogViaMQTTThreadRoutine(void *data)
{
	...
	connectStatus = mosquitto_connect(mqttContext->mosq, appConfig.LogViaMQTTBrokerIP,
	...
	while (!mqttContext->Stop) {
		...
		RingBufferFetch(mqttContext->MQTTLogRingBuffer, (unsigned char *)&stats,
				sizeof(stats), &logDataLen);
		...
		for (int i = 0; i < nofReadElements; i++)
			LogViaMQTTAddTrafficClass(mqttContext->mosq,
...
struct LogViaMQTTThreadContext *LogViaMQTTThreadCreate(void)
{
	...
	ret = CreateRtThread(&mqttContext->MQTTLogTaskId, "LoggerGraph",
			     appConfig.LogViaMQTTThreadPriority, appConfig.LogViaMQTTThreadCpu,
			     LogViaMQTTThreadRoutine, mqttContext);
