...
void InsertVlanTag(void *buffer, size_t len, uint16_t vlanTCI)
{
	...
	memmove(buffer + 2 * ETH_ALEN + sizeof(struct VLANHeader), buffer + 2 * ETH_ALEN,
		len - 2 * ETH_ALEN);
...
static void InitializeSecureProfinetFrame(enum SecurityMode mode, unsigned char *frameData,
					  size_t frameLength, const unsigned char *source,
...
static void InitializeRtProfinetFrame(unsigned char *frameData, size_t frameLength,
			     ...
			     uint16_t vlanTCI, uint16_t frameId)
{
	...
	case SECURITY_MODE_NONE:
		InitializeRtProfinetFrame(frameData, frameLength, source, destination,
					  payloadPattern, payloadPatternLength, vlanTCI, frameId);
		break;
	case SECURITY_MODE_AE:
	case SECURITY_MODE_AO:
		InitializeSecureProfinetFrame(mode, frameData, frameLength, source, destination,
					      payloadPattern, payloadPatternLength, vlanTCI,
					      frameId);
		break;
	}
}
...
int PrepareFrameForTx(const struct PrepareFrameConfig *frameConfig)
{
	...
	if (frameConfig->Mode == SECURITY_MODE_NONE) {
		...
		struct ReferenceMetaData *metaData =
			(struct ReferenceMetaData *)(frameConfig->FrameData +
						     frameConfig->MetaDataOffset);

		SequenceCounterToMetaData(metaData, frameConfig->SequenceCounter,
					  frameConfig->NumFramesPerCycle);
	...
	/* mode == AO is PROFINET specific */
		...
		SequenceCounterToMetaData(&srt->MetaData, frameConfig->SequenceCounter,
					  frameConfig->NumFramesPerCycle);
		...
		sizeOfAadData =
			frameConfig->FrameLength - sizeof(*eth) - sizeof(struct SecurityChecksum);
		beginOfSecurityChecksum =
			frameConfig->FrameData +
			(frameConfig->FrameLength - sizeof(struct SecurityChecksum));
		return SecurityEncrypt(frameConfig->SecurityContext, NULL, 0, beginOfAadData,
				       sizeOfAadData, (unsigned char *)&iv, NULL,
				       beginOfSecurityChecksum);
	}
	/* mode == AE is PROFINET specific too */
		...
		return SecurityEncrypt(frameConfig->SecurityContext, frameConfig->PayloadPattern,
				       frameConfig->PayloadPatternLength, beginOfAadData,
				       sizeOfAadData, (unsigned char *)&iv, beginOfCiphertext,
				       beginOfSecurityChecksum);
	}
}
...
void PrepareOpenssl(struct SecurityContext *context)
{
	...
	SecurityEncrypt(context, NULL, 0, dummyFrame,
			sizeof(dummyFrame) - sizeof(struct SecurityChecksum), iv, NULL,
			dummyFrame + sizeof(dummyFrame) - sizeof(struct SecurityChecksum));

	SecurityDecrypt(
		context, NULL, 0, dummyFrame, sizeof(dummyFrame) - sizeof(struct SecurityChecksum),
