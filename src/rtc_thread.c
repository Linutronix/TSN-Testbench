...
static void RtcInitializeFrames(unsigned char *frameData, size_t numFrames,
				const unsigned char *source, const unsigned char *destination)
{
	size_t i;

	for (i = 0; i < numFrames; ++i)
		InitializeProfinetFrame(appConfig.RtcSecurityMode,
					frameData + i * RTC_TX_FRAME_LENGTH, RTC_TX_FRAME_LENGTH,
					source, destination, appConfig.RtcPayloadPattern,
					appConfig.RtcPayloadPatternLength,
					appConfig.RtcVid | RTC_PCP_VALUE << VLAN_PCP_SHIFT, 0x8000);
}

static void RtcSendFrame(const unsigned char *frameData, size_t frameLength,
			 size_t numFramesPerCycle, int socketFd)
{
	...
	StatFrameSent(RTC_FRAME_TYPE, sequenceCounter);
}

static void RtcGenAndSendFrame(struct SecurityContext *securityContext, unsigned char *frameData,
			       ...
			       uint64_t sequenceCounter)
{
	...
	frameConfig.PayloadPattern = frameData + 1 * RTC_TX_FRAME_LENGTH +
				   ...
				   uint64_t sequenceCounter, uint32_t *frameNumber)
{
	...
	xdp.PayloadPattern = txFrameData + 1 * RTC_TX_FRAME_LENGTH +
			     sizeof(struct VLANEthernetHeader) +
			     sizeof(struct ProfinetSecureHeader);
	...
	while (!threadContext->Stop) {
		...
		if (!mirrorEnabled) {
			for (i = 0; i < appConfig.RtcNumFramesPerCycle; ++i)
				RtcGenAndSendFrame(securityContext, threadContext->TxFrameData,
			...
			for (i = 0; i < len / appConfig.RtcFrameLength; ++i)
				RtcSendFrame(receivedFrames + i * appConfig.RtcFrameLength,
					     appConfig.RtcFrameLength,
					     appConfig.RtcNumFramesPerCycle, socketFd);
		}
...
static void *RtcXdpTxThreadRoutine(void *data)
{
	...
	RtcInitializeFrames(frameData, XSK_RING_CONS__DEFAULT_NUM_DESCS, source,
			    appConfig.RtcDestination);
	RtcInitializeFrames(threadContext->TxFrameData, 2, source, appConfig.RtcDestination);
	...
	while (!threadContext->Stop) {
		 ...
		 */
		if (!mirrorEnabled) {
			RtcGenAndSendXdpFrames(securityContext, xsk, threadContext->TxFrameData,
			...
			for (i = sequenceCounter; i < sequenceCounter + received; ++i)
				StatFrameSent(RTC_FRAME_TYPE, i);
...
static int RtcRxFrame(void *data, unsigned char *frameData, size_t len)
{
	...
	if (appConfig.RtcSecurityMode == SECURITY_MODE_NONE) {
		...
		PrepareIv((const unsigned char *)appConfig.RtcSecurityIvPrefix, sequenceCounter,
			  &iv);
	...
	frameIdMismatch = frameId != 0x8000;

	StatFrameReceived(RTC_FRAME_TYPE, sequenceCounter, outOfOrder, payloadMismatch,
			  frameIdMismatch);
...
static void *RtcXdpRxThreadRoutine(void *data)
{
	...
	while (!threadContext->Stop) {
		...
		pthread_mutex_lock(&threadContext->XdpDataMutex);
		received = XdpReceiveFrames(xsk, frameLength, mirrorEnabled, RtcRxFrame,
					    threadContext);
...
int RtcThreadsCreate(struct ThreadContext *threadContext)
{
	...
	if (appConfig.RtcXdpEnabled) {
		...
		threadContext->Xsk = XdpOpenSocket(
			appConfig.RtcInterface, appConfig.ApplicationXdpProgram,
	...
	if (appConfig.RtcRxMirrorEnabled && !appConfig.RtcXdpEnabled) {
		...
		threadContext->MirrorBuffer =
			RingBufferAllocate(RTC_TX_FRAME_LENGTH * appConfig.RtcNumFramesPerCycle);
	...
	if (appConfig.RtcSecurityMode != SECURITY_MODE_NONE) {
		threadContext->TxSecurityContext = SecurityInit(
			appConfig.RtcSecurityAlgorithm, (unsigned char *)appConfig.RtcSecurityKey);
	...
	ret = CreateRtThread(&threadContext->TxTaskId, "RtcTxThread", appConfig.RtcTxThreadPriority,
			     ...
			     appConfig.RtcXdpEnabled ? RtcXdpTxThreadRoutine : RtcTxThreadRoutine,
			     threadContext);
	if (ret) {
	...
	ret = CreateRtThread(&threadContext->RxTaskId, "RtcRxThread", appConfig.RtcRxThreadPriority,
			     ...
			     appConfig.RtcXdpEnabled ? RtcXdpRxThreadRoutine : RtcRxThreadRoutine,
			     threadContext);
	if (ret) {
