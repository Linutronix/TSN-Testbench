...
static int rtc_rx_frame(void *data, unsigned char *frame_data, size_t len)
{
	struct thread_context *thread_context = data;
	const struct traffic_class_config *rtc_config = thread_context->conf;
	const unsigned char *expected_pattern = (const unsigned char *)rtc_config->payload_pattern;
	struct security_context *security_context = thread_context->rx_security_context;
	const size_t expected_pattern_length = rtc_config->payload_pattern_length;
	const size_t num_frames_per_cycle = rtc_config->num_frames_per_cycle;
	const bool mirror_enabled = rtc_config->rx_mirror_enabled;
	const bool ignore_rx_errors = rtc_config->ignore_rx_errors;
	size_t expected_frame_length = rtc_config->frame_length;
	...
	if (rtc_config->security_mode == SECURITY_MODE_NONE) {
	...
	} else if (rtc_config->security_mode == SECURITY_MODE_AO) {
		...
		prepare_iv((const unsigned char *)rtc_config->security_iv_prefix, sequence_counter,
			   &iv);
