...
static struct sock_filter RtaFrameFilter[] = {
    {0x28, 0, 0, 0x0000000c}, {0x15, 0, 5, 0x00008892}, {0x20, 0, 0, 0xfffff02c}, {0x15, 0, 3, 0x00001234},
    {0x28, 0, 0, 0x0000000e}, {0x15, 0, 1, 0x0000fc01}, {0x06, 0, 0, 0xffffffff}, {0x06, 0, 0, 0000000000},
};
...
static int CreateRawSocket(const char *ifName, int socketPriority)
{
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_PRIORITY, &socketPriority, sizeof(socketPriority));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_index;
...
int CreateTSNHighSocket(void)
{
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &tsnHighFilterProgram, sizeof(tsnHighFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
{
    const struct sock_fprog tsnLowFilterProgram = {.len = ARRAY_SIZE(TsnLowFrameFilter), .filter = TsnLowFrameFilter};
    struct sock_txtime skTxtime;
    int socketFd, ret;
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &tsnLowFilterProgram, sizeof(tsnLowFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
{
    const struct sock_fprog rtcFilterProgram = {.len = ARRAY_SIZE(RtcFrameFilter), .filter = RtcFrameFilter};
    int socketFd, ret;
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &rtcFilterProgram, sizeof(rtcFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
{
    const struct sock_fprog rtaFilterProgram = {.len = ARRAY_SIZE(RtaFrameFilter), .filter = RtaFrameFilter};
    int socketFd, ret;
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &rtaFilterProgram, sizeof(rtaFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
{
    const struct sock_fprog dcpFilterProgram = {.len = ARRAY_SIZE(DcpFrameFilter), .filter = DcpFrameFilter};
    int socketFd, ret;
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &dcpFilterProgram, sizeof(dcpFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
{
    const struct sock_fprog lldpFilterProgram = {.len = ARRAY_SIZE(LldpFrameFilter), .filter = LldpFrameFilter};
    int socketFd, ret;
    ...
    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &lldpFilterProgram, sizeof(lldpFilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
int CreateGenericL2Socket(void)
{
    ...
    GenericL2FrameFilter[3].k = appConfig.GenericL2Vid | appConfig.GenericL2Pcp << VLAN_PCP_SHIFT;

    ret = setsockopt(socketFd, SOL_SOCKET, SO_ATTACH_FILTER, &genericL2FilterProgram, sizeof(genericL2FilterProgram));
    if (ret < 0)
    {
        perror("setsockopt() failed");
        goto err_filter;
...
static int DnsLookup(const char *host, const char *port, struct sockaddr_storage *addr, int *socketFd)
{
