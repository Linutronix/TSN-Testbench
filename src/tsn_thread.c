...
static int tsn_rx_frame(void *data, unsigned char *frame_data, size_t len)
{
	struct thread_context *thread_context = data;
	const struct traffic_class_config *tsn_config = thread_context->conf;
	const unsigned char *expected_pattern = (const unsigned char *)tsn_config->payload_pattern;
	struct security_context *security_context = thread_context->rx_security_context;
	const size_t expected_pattern_length = tsn_config->payload_pattern_length;
	const bool mirror_enabled = tsn_config->rx_mirror_enabled;
	const bool ignore_rx_errors = tsn_config->ignore_rx_errors;
	size_t expected_frame_length = tsn_config->frame_length;
	...
	if (tsn_config->security_mode == SECURITY_MODE_NONE) {
		...
		sequence_counter = meta_data_to_sequence_counter(&rt->meta_data,
								 tsn_config->num_frames_per_cycle);
	...
	} else if (tsn_config->security_mode == SECURITY_MODE_AO) {
		...
		prepare_iv((const unsigned char *)tsn_config->security_iv_prefix, sequence_counter,
			   &iv);
