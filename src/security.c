...
/*
 * Code inspired by https://wiki.openssl.org/index.php/EVP_Authenticated_Encryption_and_Decryption and
 *                  https://github.com/openssl/openssl/discussions/22161
 *
 * Notes for the algorithms used: The IV size is 12 bytes. The tag len is always 16 bytes. And Input equals Output
 * length. Furthermore, PROFINET supports Authentification Only. In that case plaintext respectively ciphertext is NULL.
 */
                    ...
                    unsigned char *ciphertext, unsigned char *tag)
{
    ...
    if (plaintext)
    {
        if (1 != EVP_EncryptUpdate(context->Ctx, ciphertext, &len, plaintext, plaintextLength))
            goto out;
    }
