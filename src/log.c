...
void LogMessage(enum LogLevel level, const char *format, ...)
{
    ...
    written = snprintf(p, len, "[%8ld.%9ld]: [%s]: ", time.tv_sec, time.tv_nsec, LogLevelToString(level));
    p += written;
    len -= written;
...
static void LogAddTrafficClass(const char *name, enum StatFrameType frameType, char **buffer, size_t *length)
{
    const struct Statistics *stat = &GlobalStatistics[frameType];
    int written;

    written = snprintf(*buffer, *length,
                       "%sSent=%" PRIu64 " | %sReceived=%" PRIu64 " | %sRttMin=%" PRIu64 " [us] | %sRttMax=%" PRIu64
                       " [us] | %sRttAvg=%lf [us] | ",
                       name, stat->FramesSent, name, stat->FramesReceived, name, stat->RoundTripMin, name,
                       stat->RoundTripMax, name, stat->RoundTripAvg);
    ...
    *length -= written;
    ...
    {
        written = snprintf(*buffer, *length, "%sRttOutliers=%" PRIu64 " | ", name, stat->RoundTripOutliers);
        *buffer += written;
        *length -= written;
    }
}
...
static void *LogThreadRoutine(void *data)
{
    ...
    while (!logContext->Stop)
    {
        ...
        if (CONFIG_IS_TRAFFIC_CLASS_ACTIVE(GenericL2))
            LogAddTrafficClass(appConfig.GenericL2Name, GENERICL2_FRAME_TYPE, &p, &statMessageLength);

        LogMessage(LOG_LEVEL_INFO, "%s\n", statMessage);

        /* Fetch data */
        RingBufferFetch(logContext->LogRingBuffer, logContext->LogData, LOG_BUFFER_SIZE, &logDataLen);

        /* Write down to disk */
        if (logDataLen > 0)
        {
            fwrite(logContext->LogData, sizeof(char), logDataLen, logContext->FileHandle);
...
struct LogThreadContext *LogThreadCreate(void)
{
    ...
    ret = CreateRtThread(&logContext->LogTaskId, "Logger", appConfig.LogThreadPriority, appConfig.LogThreadCpu,
                         LogThreadRoutine, logContext);
    if (ret)
        goto err_thread;
